...
| ------------------------- | ---------------- | ------------ | ------------------------------------------------------- |
| `מינימרקט מנצור ובניו`    | מזון וסופר       | אחר          | In few-shot; model ignores example                      |
| `שטראוס מים בע"מ הו"ק`    | חשבונות ושירותים | אחר          | In few-shot; model ignores example                      |
| `שרותי בריאות כללית הו"ק` | בריאות           | אחר          | In few-shot; model ignores example                      |
| `עמלת סמס חבילה בסיסית`   | תשלום כ. אשראי   | אחר          | In few-shot; model ignores example                      |
| `פרשמרקט מרום נווה`       | מזון וסופר       | קניות וביגוד | Batch 3 outlier — "מרום" triggered shopping association |
...
| -------------------- | ----------- | ---------- |
| `גוגיס גריל בר`      | מסעדות וקפה | מזון וסופר |
| `המאפיה אלנבי`       | מסעדות וקפה | מזון וסופר |
| `שווארמה שאולי חדרה` | מסעדות וקפה | מזון וסופר |
| `אלונית פארק חדרה`   | רכב ודלק    | מזון וסופר |
...
| Description                 | Expected         | Predicted      |
| --------------------------- | ---------------- | -------------- |
| `HOT MOBILE הו"ק`           | חשבונות ושירותים | מזון וסופר     |
| `דוגז שווק בע"מ`            | חשבונות ושירותים | אחר            |
| `פרי טיוי`                  | חשבונות ושירותים | אחר            |
| `חברת פרטנר תקשורת בע"מ (ה` | חשבונות ושירותים | תחבורה ציבורית |

_Root cause:_ Israeli utility and telecom provider names are opaque to a general-purpose model. "HOT MOBILE" (Israeli cable/mobile), "דוגז" (gas), "פרי טיוי" (satellite TV) require Israel-specific knowledge. The "הו"ק" suffix (standing order) that signals recurring utility billing is not understood.

**3. Parking lots not recognized as רכב ודלק**

| Description                        | Expected | Predicted |
| ---------------------------------- | -------- | --------- |
| `נכסי אריאל-חניון מרכז ויצמן בע"מ` | רכב ודלק | אחר       |
| `חניון רכב די.סי בע"מ`             | רכב ודלק | אחר       |
...
| Description              | Expected      | Predicted |
| ------------------------ | ------------- | --------- |
| `מכבי חיפה אצטדיון`      | בילויים ופנאי | בריאות    |
| `יס פלאנט ראשל"צ- מזנון` | בילויים ופנאי | אחר       |
| `שולה בחצר`              | מסעדות וקפה   | אחר       |

_Root cause:_ "אצטדיון" (stadium) triggered a health association ("Maccabi" is also a health fund brand). "יס פלאנט" requires knowing it's a cinema chain. Restaurant names without chain recognition fall to "אחר".
