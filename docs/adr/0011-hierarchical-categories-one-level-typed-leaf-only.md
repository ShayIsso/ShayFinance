...
**Adjudication (BGR8/#177, PR #182):** this block is a one-time leaf→group promotion guard, not a standing cap on group membership. It fires only when the target category has no children yet (about to become a group for the first time); a category that already has children — an established group — keeps accepting new leaves underneath it regardless of its own population. This distinction only became observable once budgets started counting toward population (#177 wired the real count; earlier population sources — transactions, rules, memory entries, pending suggestions — can never land directly on a group in the first place, since assignment is leaf-only). A group carrying its own budget (subtree spend cap, §4) is an expected, deliberate combination — BGR8 offers groups in the budget picker — and must not be treated as "populated" for the purposes of blocking further membership changes. Both directions are pinned by tests: a populated _leaf_ (including one with only a budget) still blocks gaining a child; an established _group_ with its own budget still accepts one.
