...
- `llama3.1:8b` confused restaurants with supermarkets (`גוגיס גריל בר`, `המאפיה אלנבי` → `מזון וסופר`), failed on Israeli-specific telecom/utility names (`HOT MOBILE`, `דוגז`, `פרי טיוי`), and didn't recognise parking lots (`חניון רכב די.סי`) as `רכב ודלק`.
