...
| ------------------------ | ----------------------- | ---------------- | ------------------ | --------------------------------------- |
| gemini-flash-latest      | 26/50 (52%)             | 39/50 (78%) GO   | **25/50 (50%)**    | **35/50 (70%) — GO**                    |
| gemini-flash-lite-latest | 19/50 (38%)             | 32/50 (64%)      | **23/50 (46%)**    | 31/50 (62%) — NO-GO                     |
| gemini-pro-latest        | 25/50 (50%)             | 38/50 (76%)      | not run (reserved) | —                                       |

#100's gate score was the _adjudicated_ score (§1: labels encoding private context or taxonomy ambiguity measure the fixture, not the model), and its adjudication was owner-signed. That adjudication did **not** carry over — it was authored against the old taxonomy and old items — so the owner re-adjudicated the v2 misses live (2026-07-14): each of flash's 25 raw misses was ruled either "model correct under v2, historical label stale" or "label stands". The 10 accepted rulings replace the #100 `adjudications.json` (gitignored, applied by `rescore.ts`). The adjudicated column above is the gate score, as in #100.
...
The owner reviewed all 25 flash misses live and ruled **10** of them "model correct under v2, historical label stale": the two ATM-withdrawal items, the two P2P-to-person transfers labelled by remembered purpose, the barbershop item, the two cloud-storage subscription items, the payment-app merchant item, the food-supplements item, and the ice-cream parlour item. The other **15 stand as real misses** — including, conservatively, the owner's own self-transfer descriptions, the family-account transfers, and the non-profit fee, plus the sibling near-misses and genuine errors. (An eleventh ruling — the venue the model called a subscription — was initially accepted, then retracted: the owner had explicitly assigned that venue during the #133 sitting, and the earlier explicit assignment stands.)

The ruling is more conservative than this section's (b)-class estimate: 10 items accepted against the 13-candidate projection of ≈76%, keeping the personal-context transfers as label-stands while agreeing with the model on two items the pre-adjudication analysis had classed as near-misses. **Final adjudicated score: flash-latest 35/50 = 70%.**

Applying the same 10 rulings to `gemini-flash-lite-latest` (it missed all 10 items, but on two of them gave a _different_ wrong answer than the accepted one, so it gains only 8) yields **31/50 = 62% adjudicated** — below the gate on every cut; it stays out either way.
...
- **Raw does not clear the gate** (flash 50%, lite 46%) — but per §1 the gate has always been measured on the adjudicated score, because raw holds the model to labels that encode private context. This re-run reproduces that gap: #100 was 52% raw → 78% adjudicated; v2 is 50% raw → **70% owner-adjudicated**.
- **`gemini-flash-latest` clears the gate at 70% — GO reaffirmed.** The descriptions made the model's answers _more_ v2-aligned; the residual genuine-error rate is ~8%. The taxonomy-v2 prompt is a net improvement in model behaviour even though the raw number, measured against un-migrated historical labels, does not show it. Note the owner adjudicated more conservatively than #100's basis (10 rulings vs the 13-candidate ≈76% projection above); the gate clears exactly at the ≥70% bar, on strictly owner-conservative rulings.
...
- Separately from the model decision, the stale-under-v2 labels the adjudication confirmed (barbershop → `בריאות וטיפוח`, cloud storage → `מנויים`) were historical manual labels the migration left untouched; the owner approved a one-time relabel, applied 2026-07-14 (10 transactions moved to their v2 homes, `category_source='user'`).
